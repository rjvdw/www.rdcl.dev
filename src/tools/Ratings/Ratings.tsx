...
export const Ratings = () => {
  ...
  const percentage = nrTotalNr
    ? ((100 * nrPositiveNr) / nrTotalNr).toFixed(2)
    : '∞'
  ...
  return (
    <>
      ...
      <rdcl-input-grid suffix>
        <label data-start={1} htmlFor={`${id}:rating-positive`}>
          Good reviews
        </label>
        <input
          ...
          onChange={(event: ChangeEvent<HTMLInputElement>) =>
            setNrPositive(event.target.value === '' ? '' : +event.target.value)
          }
        />

        <label data-start={1} htmlFor={`${id}:rating-total`}>
          Total reviews
        </label>
        <input
          id={`${id}:rating-total`}
          ...
          value={nrTotal}
          onChange={(event: ChangeEvent<HTMLInputElement>) =>
            setNrTotal(event.target.value === '' ? '' : +event.target.value)
          }
        />

        <hr data-span={2} />

        <label data-start={1} htmlFor={`${id}:rating-percentage`}>
          Percentage
        </label>
        <input
        ...
        <label data-start={1} htmlFor={`${id}:rating-score`}>
          Score
        </label>
        <input
          id={`${id}:rating-score`}
