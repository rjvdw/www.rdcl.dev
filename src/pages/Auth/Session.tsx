...
export const Session = () => {
  ...
  return (
    <>
      ...
      <h1>Session</h1>
      {jwt ? <RenderJwt jwt={jwt} /> : <p>No session</p>}
    </>
  )
}
...
const JwtClaim: FunctionComponent<{ children: string }> = ({
  children: claim,
}) => (
  <th>
    {JWT_CLAIMS[claim] ? <abbr title={JWT_CLAIMS[claim]}>{claim}</abbr> : claim}
  </th>
)

const JwtValue: FunctionComponent<{ claim: string; children: unknown }> = ({
  claim,
  children: value,
}) => {
