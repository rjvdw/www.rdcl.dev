...
import dev.rdcl.www.server.util.Resources;
...
public class JwtService {
     ...
     *
     * @param user The identity for which the authorization token is issued.
     * @return An authorization token.
     */
    public String issueAuthToken(UserEntity user) {
        return Jwt
            .issuer(jwtProperties.issuer())
            .subject(user.getId().toString())
            .upn(user.getEmail())
            .groups(Set.of("user"))
            .preferredUserName(user.getDisplayName())
