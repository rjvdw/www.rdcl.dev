...
exports.App = class App {
  ...
  getHandler() {
    ...
    this.app.use((err, req, res, next) => {
      if (err instanceof faunadb.errors.BadRequest) {
        console.warn('FaunaDB Error', err)
        res.status(400).json({ reason: err.description })
      } else {
...
exports.beforeAfterWhere = (result, where) => ({
  before: result?.before?.find(where),
  after: result?.after?.find(where),
})
